'''
    ...
    [1] W. W. Grabowski and T. L. Clark. ‘Cloud-Environment Interface
        Instability: Rising Thermal Calculations in Two Spatial Dimen-
        sions’. In: J. Atmos. Sci. 48.4 (1991), pp. 527–546.
    ...
    [2] G. Doms et al. COSMO-Model Version 6.00: A Description of the
        ...
        trizations. Tech. rep. COSMO Consortium for Small-Scale Modelling,
...
parser = argparse.ArgumentParser(description='Rising temerature bubble with rain',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('-hmax', '--mesh_size', type=float, default=50, help='Mesh size')
...
pickle_name = f'rising_thermal_rain_dg_ssprk43_pert_artdiff{scale_diff}'
...
x_c, z_c = domain_w / 2, 800  # Center of humidity bubble
...
if struc_mesh:
    ...
    geo.Append(["line", pnums[1], pnums[2]], bc='right')
    geo.Append(["line", pnums[3], pnums[2]], leftdomain=0, rightdomain=1,
               copy=bot)
    geo.Append(["line", pnums[3], pnums[0]], bc='left')
    mesh = Mesh(geo.GenerateMesh(maxh=h_max, quad_dominated=quad_mesh))

X0 = L2(mesh, order=0)
X1 = L2(mesh, order=order)**6
X2 = L2(mesh, order=order)**3
X = L2(mesh, order=order)**15
...
def F(U):
    rho_d_per, rho_m_per, rho_r_per, rhoU1, rhoU2, E_per, _rho_v, _rho_c, \
        T, _rho_d_0, _rho_m_0, _rho_r_0, E_0, _rho_v_0, T_0 = U
...
def G(U):
    ...
    G42 = 0
    G5 = rhoU1 * g
    return CF((G1, G2, G3, G41, G42, G5, 0, 0, 0, 0, 0, 0, 0, 0, 0))
